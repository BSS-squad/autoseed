...
> Историческое техническое задание прежнего приложения Pages. После
> функционального перехода пользовательские сценарии принадлежат единому сайту,
> а этот репозиторий публикует только совместимое перенаправление. Актуальная
> схема описана в [README](../README.md).
...
1. Использовать единый порядок `1 -> 2 -> 3`
   (`Mix -> Spec Ops -> Invasion`) в любое время суток.
2. Пропускать недоступные серверы, серверы без `isSeedCandidate` и серверы
   с онлайном на уровне лимита.
3. Если в заданном порядке нет кандидата, выбрать самый населённый
   подходящий seed-сервер.

### Формализация

1. Рассматриваются только серверы, где
   `online === true` и `isSeedCandidate === true`.
2. Исключаются серверы с `playerCount >= maxSeedPlayers`.
3. Выбирается первый подходящий сервер в строгом порядке `1 -> 2 -> 3`.
4. Если перечисленных кандидатов нет, выбирается наиболее населённый
   подходящий seed-сервер.
5. Если кандидатов нет, переход не выполняется.

### Актуальная policy BSS

- приоритет: `1 -> 2 -> 3` (`Mix -> Spec Ops -> Invasion`) в любое время
- `maxSeedPlayers`: `80`
