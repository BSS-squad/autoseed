...
`raffles.budget` описывает общий production-банк серии и может повторяться в snapshots разных exporter-ов. Страница победителей должна брать его из одного основного raffle snapshot-а, а не суммировать по всем серверам. `raffles.campaign` нужен для отображения планируемой серии до первого активного розыгрыша.
...
- показывать план серии из `raffles.campaign` и общий банк из одного raffle snapshot-а без суммирования одинакового бюджета между exporter-ами.
...
- frontend отображает планируемую серию из `raffles.campaign` и не умножает общий `raffles.budget` на количество exporter-ов.
