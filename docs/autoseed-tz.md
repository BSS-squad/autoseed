  ...
  "servers": [
    {
      ...
      "joinLink": "https://example.com/api/pub/join-link-result",
      "teams": [
        {
          "id": 1,
          "name": "Team 1",
...
- exporter отдаёт read-only `snapshot`/`healthz`/`events`, включая roster и playtime summary.
...
5. После этого открыть GitHub Pages frontend и проверить realtime-подписку, preflight-check и redirect.
