...
`raffles.budget` описывает общий production-банк серии и может повторяться в snapshots разных exporter-ов, поэтому frontend не суммирует его по серверам. `raffles.campaigns[]` содержит несколько планов, а legacy `raffles.campaign` остаётся совместимым источником одной кампании. Одинаковые кампании дедуплицируются; будущие выводятся как уведомления, текущая — как карточка `Серия`.
...
- показывать будущие серии из `raffles.campaigns[]`/`raffles.campaign` отдельными дедуплицированными уведомлениями, а карточку `Серия` — только во время кампании.
