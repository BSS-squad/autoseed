...
1. Определить дневной или ночной порядок по времени политики.
2. Ночью использовать `3 -> 2 -> 1` (`Invasion -> Spec Ops -> Mix`).
3. Днём использовать `1 -> 2 -> 3` (`Mix -> Spec Ops -> Invasion`).
4. Пропускать недоступные серверы, серверы без `isSeedCandidate` и серверы
   с онлайном на уровне лимита.
5. Если в заданных порядках нет кандидата, выбрать самый населённый
   подходящий seed-сервер.

### Формализация

1. Если локальное время политики попадает в диапазон `00:00–08:00`,
   применяется `nightPriorityOrder`; начало включительно, конец исключительно.
2. В остальное время применяется `priorityOrder`.
3. В обоих режимах рассматриваются только серверы, где
   `online === true` и `isSeedCandidate === true`.
4. Исключаются серверы с `playerCount >= maxSeedPlayers`.
5. Выбирается первый подходящий сервер в активном строгом порядке.
6. Если перечисленных кандидатов нет, выбирается наиболее населённый
   подходящий seed-сервер.
7. Если кандидатов нет, переход не выполняется.
...
- `nightPriorityOrder`: `3 -> 2 -> 1` (`Invasion -> Spec Ops -> Mix`)
