...
- предпочтительный формат: готовый lobby link из `Squadbrowser API` или fallback `steam://connect/IP:PORT`.
...
- exporter отдаёт read-only `snapshot`/`healthz`, включая roster и playtime summary.
