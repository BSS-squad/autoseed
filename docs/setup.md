...
Этот JSON попадает в публичный frontend bundle. Никаких приватных ключей, auth token и SSH-данных туда класть нельзя. Сборка разрешает только задокументированные поля и публичные `http`/`https` URL без логина, пароля, query-параметров и фрагмента; неподдерживаемые поля блокируют выпуск.
