...
Тогда в интерфейсе появится отдельный тестовый режим для ручного end-to-end прогона `2 -> 1`, а боевой режим останется отдельным. Задержку follow-up можно локально перекрыть прямо на странице; override хранится в `localStorage` конкретного браузера.
