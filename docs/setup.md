...
Страница победителей открывается по `https://bss-squad.github.io/autoseed/#winners`. Она читает только публичное поле `servers[].raffles` из snapshot. Для планируемой серии UI использует `raffles.campaign`, а банк берёт из одного основного raffle snapshot-а, потому что production SQL-бюджет общий для всех SquadJS exporter-ов и не должен суммироваться по `squadjs1/2/3`. Если Raffle-плагин на production ещё не включён и exporter отдаёт `raffles: null`, страница показывает empty state без ошибок.
