...
Важно: это публичный клиентский конфиг. Даже если он подставляется через GitHub Secrets, после билда значения становятся видимыми в браузере. Сборка принимает только задокументированные поля и публичные `http`/`https` URL без логина, пароля, query-параметров и фрагмента. Не кладите туда приватные ключи, токены или другие учётные данные.
